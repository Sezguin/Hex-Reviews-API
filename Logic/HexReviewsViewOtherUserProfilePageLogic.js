...
$(document).ready(function () {
    ...
    $('#myReviewsButton').click(function () {
        goToViewOtherReviewsPage(globalUserID);
    });
...
function getReviews(callback) {
    $.ajax({
        url: GlobalURL + '/reviews/' + globalUserID,
        type: 'GET',
        success: function(result) {
            if(!result) {
                console.log("User has no reviews.");
                document.getElementById("yourLatestReviews").innerHTML = "You have no reviews...";
                document.getElementById("mostPopularReviews").innerHTML = "";
            } else {
                callback(result, displayReviews, "latest");
                callback(result, displayReviews, "popular");
                document.getElementById("yourLatestReviews").innerHTML = "Latest Reviews";
                document.getElementById("mostPopularReviews").innerHTML = "Most Popular Reviews";
            }
        }
    });
}
...
function sortReviewArray(reviews, callback, sorting) {
    ...
    if(sorting == "latest") {
        ...
        console.log("Sorted review: " + reviews);
        ...
        for(var i = 0; i < 3; i++) {
            try {
                ...
                console.log("Ran out of reviews to show.")
            }
        }
    }
